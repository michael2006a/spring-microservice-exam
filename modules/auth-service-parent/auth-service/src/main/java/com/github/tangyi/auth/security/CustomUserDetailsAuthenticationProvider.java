...
public class CustomUserDetailsAuthenticationProvider extends
  ...
  protected void additionalAuthenticationChecks(UserDetails userDetails,
    ...
    if (!passwordEncoder.matches(presentedPassword, userDetails.getPassword())) {
      log.debug("认证失败: 密码错误.");
      throw new BadCredentialsException(
          messages.getMessage("AbstractUserDetailsAuthenticationProvider.badCredentials", "密码错误."));
    }
  }
  ...
  protected UserDetails retrieveUser(String username,
    ...
    try {
      ...
      loadedUser = this.userDetailsService
          .loadUserByIdentifierAndTenantCode(authentication.getPrincipal().toString(),
              TenantContextHolder.getTenantCode());
