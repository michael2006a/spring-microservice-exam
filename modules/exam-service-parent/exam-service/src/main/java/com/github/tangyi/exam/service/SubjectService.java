...
public class SubjectService {
   ...
   */
  @Transactional
  public SubjectDto getNextByCurrentIdAndType(Long examinationId, Long previousSubjectId,
      Integer type, Integer nextType) {
    return subjectService(type)
  ...
  public PageInfo<SubjectDto> findPage(PageInfo pageInfo, SubjectDto subjectDto) {
    ...
    examinationSubject.setExaminationId(subjectDto.getExaminationId());
    PageInfo<ExaminationSubject> examinationSubjectPageInfo = examinationSubjectService
        .findPage(pageInfo, examinationSubject);
    ...
    if (CollectionUtils.isNotEmpty(examinationSubjectPageInfo.getList())) {
      examinationSubjectPageInfo.getList().forEach(tempExaminationSubject -> {
        SubjectDto tempSubjectDto = subjectService(tempExaminationSubject.getType())
            .getSubject(tempExaminationSubject.getSubjectId());
  ...
  public int physicalDeleteAll(Long[] ids) {
    if (ArrayUtils.isNotEmpty(ids)) {
      ...
      for (Long id : ids) {
        ...
        List<ExaminationSubject> examinationSubjects = examinationSubjectService
            .findListBySubjectId(examinationSubject);
        if (CollectionUtils.isNotEmpty(examinationSubjects)) {
  ...
  public int importSubject(List<SubjectDto> subjects, Long examinationId, Long categoryId) {
    int updated = 0;
    String creator = SysUtil.getUser(), sysCode = SysUtil.getSysCode(), tenantCode = SysUtil
        .getTenantCode();
    // 暂时循环遍历保存
  ...
  public List<SubjectDto> findSubjectDtoList(List<ExaminationSubject> examinationSubjects,
    ...
    if (idMap.containsKey(SubjectTypeEnum.CHOICES.name())) {
      List<SubjectChoices> subjectChoicesList = subjectChoicesService
    ...
    if (idMap.containsKey(SubjectTypeEnum.SHORT_ANSWER.name())) {
      List<SubjectShortAnswer> subjectShortAnswers = subjectShortAnswerService
  ...
  public List<SubjectDto> export(Long[] ids, Long examinationId, Long categoryId) {
    ...
    if (ArrayUtils.isNotEmpty(ids)) {
      for (Long id : ids) {
        examinationSubject.setSubjectId(id);
        examinationSubjects
            .addAll(examinationSubjectService.findListBySubjectId(examinationSubject));
      }
