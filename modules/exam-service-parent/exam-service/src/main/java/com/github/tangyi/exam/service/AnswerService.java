...
public class AnswerService extends CrudService<AnswerMapper, Answer> {
   ...
   */
  @Transactional
  public SubjectDto saveAndNext(AnswerDto answerDto, Integer type, Long nextSubjectId,
      Integer nextSubjectType) {
  ...
  /**
   * 通过mq异步处理 1. 先发送消息 2. 发送消息成功，更新提交状态，发送失败，返回提交失败 3. 消费消息，计算成绩
   *
   * @param answer answer
   * @return boolean
   * @author tangyi
  ...
  public StartExamDto start(ExaminationRecord examRecord) {
    ...
    if (examRecordService.insert(examRecord) > 0) {
      ...
      // 根据题目ID，类型获取第一题的详细信息
      SubjectDto subjectDto = subjectService
          .findFirstSubjectByExaminationId(examRecord.getExaminationId());
   ...
   */
  @Transactional
  public SubjectDto subjectAnswer(Long subjectId, Long examRecordId, String userCode,
      String sysCode,
    ...
    if (nextSubjectId != null) {
      ...
      subject = subjectService.getNextByCurrentIdAndType(examRecord.getExaminationId(), subjectId,
          examinationSubjectPageInfo.getList().get(0).getType(), nextType);
    }
    if (subject == null) {
  ...
  private Map<String, List<Answer>> distinctAnswer(List<Answer> answers) {
    ...
    answers.stream().collect(Collectors.groupingBy(Answer::getType, Collectors.toList()))
        .forEach((type, temp) -> {
          // 匹配类型
  ...
  private void submitChoicesAnswer(List<Answer> choicesAnswer, ExaminationRecord record) {
    // 查找题目信息
    List<SubjectDto> subjects = subjectService.findListById(SubjectTypeEnum.CHOICES.getValue(),
        choicesAnswer.stream().map(Answer::getSubjectId).distinct().toArray(Long[]::new));
    // 保存答题正确的题目分数
    ...
    choicesAnswer.forEach(tempAnswer -> {
      ...
      subjects.stream()
          // 题目ID、题目答案匹配
          .filter(
              tempSubject -> tempSubject.getId().equals(tempAnswer.getSubjectId()) && tempSubject
   ...
   */
  public AnswerDto answerInfo(Long recordId, Long currentSubjectId, Integer nextSubjectType,
      Integer nextType) {
    ExaminationRecord record = new ExaminationRecord();
    ...
    if (currentSubjectId == null) {
      ...
      if (AnswerConstant.NEXT.equals(nextType)) {
        ...
        examinationSubject = examinationSubjectService
            .findByExaminationIdAndSubjectId(examinationSubject);
      }
      if (examinationSubject == null) {
        throw new CommonException("ID为" + currentSubjectId + "的题目不存在");
      }
      // 查询题目的详细信息
      subjectDto = subjectService
          .get(examinationSubject.getSubjectId(), examinationSubject.getType());
    }
    ...
    answerDto.setDuration(
        ExamRecordUtil.getExamDuration(userAnswer.getStartTime(), userAnswer.getEndTime()));
    return answerDto;
  }
  ...
  public Boolean completeMarking(ExaminationRecord examRecord) {
    ...
    if (CollectionUtils.isNotEmpty(answers)) {
      ...
      log.debug("批改完成，用户名：{}，考试ID：{}，总分：{}，耗时：{}ms", examRecord.getCreator(),
          examRecord.getExaminationId(),
