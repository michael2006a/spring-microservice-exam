...
public class MenuService extends CrudService<MenuMapper, Menu> {
  ...
  public int update(Menu menu) {
    ...
    if (tenantCode.equals(SecurityConstant.DEFAULT_TENANT_CODE) || tenantCode
        .equals(menu.getTenantCode())) {
  ...
  private List<Menu> mergeMenu(List<Menu> defaultMenus, List<Menu> tenantMenus) {
    ...
    tenantMenus.forEach(tenantMenu -> {
      Optional<Menu> exist = userMenus.stream()
          .filter(
              userMenu -> userMenu.getName().equals(tenantMenu.getName()) && userMenu.getParentId()
   ...
   */
  @Transactional
  public void updateRoleMenu(Long menuId, List<Long> childrenMenuIds, String userCode,
      String sysCode,
      String tenantCode) {
