...
public class FastDfsService {
  ...
  public String uploadFile(InputStream inputStream, long size, String extName,
    ...
    try {
      ...
      StorePath storePath = fastFileStorageClient
          .uploadFile(inputStream, size, extName, metaDataSet);
   ...
   * @date 2018/3/8 15:51
   */
  public String uploadAppenderFile(String groupName, InputStream inputStream, long size,
      String extName) {
    try {
      log.info("上传文件大小{}，后缀名{}", size, extName);
      StorePath storePath = appendFileStorageClient
          .uploadAppenderFile(groupName, inputStream, size, extName);
      log.info("上传文件成功，group：{}，path：{}", storePath.getGroup(), storePath.getPath());
      return storePath.getFullPath();
    } catch (Exception e) {
      log.error("上传文件失败！", e);
   ...
   * @date 2018/1/5 12:01
   */
  public String modify(String groupName, String oldPath, InputStream inputStream, long size,
      String extName) {
