...
/**
 * Swagger聚合文档 目前问题：结合动态更新路由之后，GatewayProperties获取不到新的路由列表，导致swagger-ui显示不了 解决办法：从Redis里读取路由数据
 *
 * @author tangyi
        ...
        if (routes.contains(route.getRouteId())) {
          try {
            ...
            if (CollectionUtils.isNotEmpty(routePredicateVoList)) {
              String genKey;
              RoutePredicateVo routePredicateVo = routePredicateVoList.stream().filter(
                  routePredicate -> routePredicate.getArgs()
    ...
    if (object == null) {
      ...
      if (CollectionUtils.isNotEmpty(routeList)) {
        redisTemplate.opsForValue()
            .set(CommonConstant.ROUTE_KEY, JsonMapper.getInstance().toJson(routeList));
      }
    } else {
      routeList = JsonMapper.getInstance().fromJson(object.toString(),
          JsonMapper.getInstance().createCollectionType(ArrayList.class, Route.class));
    }
    return routeList;
  }
